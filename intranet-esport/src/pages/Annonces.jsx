...
function Annonces() {
  ...
  const typeConfig = {
    "Annonce":      { color: "type-annonce", icon: <FaBullhorn />,    label: lang === "fr" ? "Annonce"      : "Announcement" },
    "Recrue":       { color: "type-recrue",  icon: <FaUserPlus />,    label: lang === "fr" ? "Recrue"       : "Signing"      },
    "Leave":        { color: "type-leave",   icon: <FaSignOutAlt />,  label: "Leave"                                         },
    "Announcement": { color: "type-annonce", icon: <FaBullhorn />,    label: "Announcement"                                  },
    "Signing":      { color: "type-recrue",  icon: <FaUserPlus />,    label: "Signing"                                       },
  };
      ...
      <div className="annonces-container">
        ...
        <div className="annonces-stats">
          <div className="annonces-stat-card">
            <FaBullhorn className="annonces-stat-icon-react" />
          ...
          <div className="annonces-stat-card annonces-stat-card--recrue">
            <FaUserPlus className="annonces-stat-icon-react annonces-icon-recrue" />
          ...
          <div className="annonces-stat-card annonces-stat-card--leave">
            <FaSignOutAlt className="annonces-stat-icon-react annonces-icon-leave" />
            <span className="annonces-stat-label">{t.annonces_leaves}</span>
            <span className="annonces-stat-value">{nbLeaves}</span>
          </div>
          <div className="annonces-stat-card">
            <FaClipboardList className="annonces-stat-icon-react" />
        ...
        <div className="annonces-filters">
          <button
            className={`annonces-filter-btn ${filterType === "tous" ? "active" : ""}`}
                ...
                <div key={a.id} className={`annonce-card ${cfg.color}`}>
                  <div className="annonce-card-top">
                    <div className="annonce-type-badge">
                      <span className="annonce-type-icon">{cfg.icon}</span>
                      <span>{cfg.label}</span>
                    </div>
        ...
        <div className="annonces-overlay" onClick={(e) => e.target === e.currentTarget && setShowAdd(false)}>
          <div className="annonces-popup">
            ...
            <div className="popup-fields">
              ...
              <input
                type="text"
                placeholder={t.annonces_titre}
                value={form.titre}
                onChange={(e) => setForm({ ...form, titre: e.target.value })}
              />
              <textarea
                placeholder={t.annonces_contenu}
                value={form.contenu}
