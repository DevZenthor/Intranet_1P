...
function Profil() {
  ...
  const categoryLabel = (cat) => {
    if (cat === "staff")   return lang === "fr" ? "Staff"   : "Staff";
    if (cat === "player")  return lang === "fr" ? "Player"  : "Player";
    if (cat === "content") return lang === "fr" ? "Content" : "Content";
    return cat || "—";
  };

  if (loading) return <section className="profil-page"><p className="profil-loading">{t.profil_loading}</p></section>;
        ...
        <div className="profil-card">
          ...
          <div className="profil-avatar-wrap">
            {avatarUrl ? (
              ...
              <div className="profil-avatar-letter">{localUser.pseudo?.charAt(0).toUpperCase()}</div>
            )}
          </div>
          ...
          <div className="profil-infos">
            ...
            <div className="profil-badges">
              ...
              {member?.category && <span className="profil-cat-badge">{categoryLabel(member.category)}</span>}
            </div>
          ...
          <div className="profil-stats">
            ...
            <div className="profil-stat">
              ...
              <div className="profil-code-wrap-display">
                <span className="profil-code-val">{showCode ? userDB?.code : "••••••••"}</span>
                <button className="profil-eye-btn" onClick={() => setShowCode(!showCode)} title={showCode ? "Masquer" : "Afficher"}>
                  {showCode ? "🙈" : "👁️"}
                </button>
              </div>
            </div>
          ...
          <button className="profil-edit-btn" onClick={openEdit}>{t.profil_edit}</button>

        </div>
      </div>

      {showEdit && (
        <div className="profil-overlay" onClick={(e) => e.target === e.currentTarget && setShowEdit(false)}>
          <div className="profil-popup">
            <h2>{t.profil_edit_title}</h2>
            <div className="popup-fields">
              <label className="profil-label">{t.profil_pseudo}</label>
              <input type="text" placeholder={t.profil_new_pseudo} value={form.pseudo} onChange={(e) => setForm({ ...form, pseudo: e.target.value })} />
              <label className="profil-label">{t.profil_new_code}</label>
              <div className="profil-code-wrap">
                <input type={showCodeEdit ? "text" : "password"} placeholder={t.profil_code_hint} value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} />
                <button className="profil-eye-btn" onClick={() => setShowCodeEdit(!showCodeEdit)} type="button">
                  {showCodeEdit ? "🙈" : "👁️"}
                </button>
              </div>
            </div>
